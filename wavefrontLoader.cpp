...
//
// bool WFObject::load(char *)
// Public function that opens the model file, reads each line (assumes 255 character line
// length), parses line, and then closes the model file.
//
bool WFObject::load(char *filename)
{
	fstream objFile;
...
void WFObject::draw()
{
	...
	{
		glNormal3f(normals[faces[f].vn1 - 1].x,
			   normals[faces[f].vn1 - 1].y,
			   normals[faces[f].vn1 - 1].z);
		glVertex3f(vertices[faces[f].v1 - 1].x,
			   vertices[faces[f].v1 - 1].y,
			   vertices[faces[f].v1 - 1].z);

		glNormal3f(normals[faces[f].vn2 - 1].x,
			   normals[faces[f].vn2 - 1].y,
			   normals[faces[f].vn2 - 1].z);
		glVertex3f(vertices[faces[f].v2 - 1].x,
			   vertices[faces[f].v2 - 1].y,
			   vertices[faces[f].v2 - 1].z);

		glNormal3f(normals[faces[f].vn3 - 1].x,
			   normals[faces[f].vn3 - 1].y,
			   normals[faces[f].vn3 - 1].z);
		glVertex3f(vertices[faces[f].v3 - 1].x,
			   vertices[faces[f].v3 - 1].y,
			   vertices[faces[f].v3 - 1].z);
	}
